...
import { allSubjects, getNoCachedSubjects } from '../redux/features/subjectSlice'
...
const Subjects = () => {
  ...
  const {noCachedSubject} = useSelector((state)=>state.subject)
    ...
    <div className='flex' >
      ...
      <div className='flex-[9]' >
        <div className='px-4 sm:px-20 py-10 border-l-[1px] border-t-[1px] shadow-lg h-full' >
          ...
          {noCachedSubject.length === 0 ? (
            <div>
              <p>There is no Subject For the moment...</p>
            </div>
          ) : (
            <div className='flex flex-col gap-4' >
            {noCachedSubject.map((s,index)=>{
              return (
                <div key={index} >
                  <Subject s={s} />
                </div>
