...
const addBinome = async (req,res) => {
    try {
        ...
        if(Me.isBinome === true && Me.hisBinome){
            return res.status(401).json({message:"You already have binome !!"})
        }
        if(myBinome.isBinome === true && myBinome.hisBinome){
            return res.status(401).json({message:"This Student has already a  binome !!"})
        }
        await Me.updateOne({isBinome:true,hisBinome:myBinome._id})
        await myBinome.updateOne({isBinome:true,hisBinome:Me._id})
        return res.status(200).json({message:"You have been added Binome succesffully !"})
    } catch (error) {
        res.status(404).json({message:error.message})
    }
}
...
const BeNoBinome = async (req,res) => {
    try {
        ...
        if(Me.isBinome === false && !Me.hisBinome){
            return res.status(401).json({message:"You already dont't have binome !!"})
        }
        if(myBinome.isBinome === false && !myBinome.hisBinome){
            return res.status(401).json({message:"This Student has already no binome !!"})
        }
...
const addTeacher = async (req,res) => {
    try {
        ...
        if(me.hisTeacher.includes(teacher._id)){
            return res.status(400).json({message:"This Teacher is already in your encadreurs"})
        }
...
const removeTeacher = async (req,res) => {
    try {
        ...
        if(!me.hisTeacher.includes(teacher._id)){
            return res.status(400).json({message:"This Teacher is not in your encadreurs list"})
        }
